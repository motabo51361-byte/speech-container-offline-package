...
若未指定 `-Port`，互動模式會詢問離線 runtime 的 host port，預設為 `5000`。例如輸入 `5001`，交付 package 內的 Compose 會產生 `"5001:5000"`；也可以直接使用 `-Port 5001` 略過詢問。此設定只控制產出的 Compose，model/license 下載 container 會使用獨立的臨時 port。Compose 的 service 與 `container_name` 不會包含 host port；STT 會自動加入 locale 以區分 `zh-TW` 與 `en-US`。所有 container 都會加入 external Docker network `meebot`。若同一台主機需要多個 Custom STT 或多個 NTTS，可在打包時用 `-RuntimeContainerName` 指定唯一名稱，不需要現場修改 YAML。
