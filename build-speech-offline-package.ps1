...
Usage:
  ...
  .\build-speech-offline-package.ps1 -Container custom-speech-to-text -RuntimeContainerName meebot-custom-stt-a
...
Notes:
  ...
  - Offline containers join the external Docker network named meebot.
  - Runtime container names do not include the host port. Use -RuntimeContainerName
    only when multiple containers would otherwise have the same default name.
...
function Get-RuntimeName([object]$Spec, [string]$ArtifactLocale, [string]$OverrideName) {
  if (-not [string]::IsNullOrWhiteSpace($OverrideName)) {
    $candidate = $OverrideName.Trim()
  } else {
    $parts = @([string]$Spec.ContainerName)
    if (-not [string]::IsNullOrWhiteSpace($ArtifactLocale)) {
      $parts += $ArtifactLocale.ToLowerInvariant()
    }
    $candidate = $parts -join "-"
  }

  $normalized = ($candidate.ToLowerInvariant() -replace "[^a-z0-9_.-]", "-").Trim("-", ".", "_")
  if ([string]::IsNullOrWhiteSpace($normalized)) {
    throw "Runtime container name is empty after normalization"
  }
...
$runtimeName = Get-RuntimeName $spec $artifactLocale $RuntimeContainerName
...
Write-Host ("Offline identity   : container={0}, network=meebot" -f $runtimeName)
...
try {
  ...
  "Offline runtime container: $runtimeName" | Out-File -FilePath $buildLogPath -Encoding utf8 -Append
  "Offline runtime network: meebot" | Out-File -FilePath $buildLogPath -Encoding utf8 -Append
  ...
  Write-RuntimeCompose $runComposePath $spec $runtimeName $Image $Memory $Cpus $Port
...
Runtime container name: $runtimeName
Runtime network name: meebot
Runtime network type: external (must exist before docker compose up)
Memory: $Memory
CPUs: $Cpus
