...
function Get-RuntimeName([object]$Spec, [string]$ArtifactLocale, [int]$HostPort) {
  $parts = @([string]$Spec.ContainerName)
  if (-not [string]::IsNullOrWhiteSpace($ArtifactLocale)) {
    $parts += $ArtifactLocale.ToLowerInvariant()
  }
  $parts += $HostPort.ToString([System.Globalization.CultureInfo]::InvariantCulture)

  return (($parts -join "-").ToLowerInvariant() -replace "[^a-z0-9_.-]", "-")
}
...
$runtimeName = Get-RuntimeName $spec $artifactLocale $Port
...
Write-Host ("Offline identity   : container={0}, network={0}-network" -f $runtimeName)
...
try {
  ...
  "Offline runtime network: ${runtimeName}-network" | Out-File -FilePath $buildLogPath -Encoding utf8 -Append
...
Runtime network name: ${runtimeName}-network
Memory: $Memory
CPUs: $Cpus
